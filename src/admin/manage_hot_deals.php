...
if (isset($_POST['toggle_hot_deal'])) {
    ...
    $stmt = $pdo->prepare("UPDATE products SET is_hot_deal = NOT is_hot_deal WHERE id = ?");
...
<body class="bg-gray-50 p-6">
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Manage Hot Deals</h1>
            ...
            <table class="min-w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Hot Deal</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($products as $product): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $product['is_hot_deal'] ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'; ?>">
                                    <?php echo $product['is_hot_deal'] ? 'Hot Deal' : 'Not Hot Deal'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form method="POST" class="inline mr-2">
                                    <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>">
                                    <button type="submit" name="toggle_hot_deal" class="<?php echo $product['is_hot_deal'] ? 'text-red-600 hover:text-red-900' : 'text-orange-600 hover:text-orange-900'; ?>">
                                        <?php echo $product['is_hot_deal'] ? 'Remove from Hot Deals' : 'Add to Hot Deals'; ?>
                                    </button>
                                </form>
